...
def validate_seg(val_loader, model_engine, global_iters, args):
    """Validate on a segmentation dataset (ReasonSeg)."""
...
def validate_vqa(
    ...
    """
    Validate VQA or Image Captioning using the same approach as pred_lisat_vqa.py
    and compute BLEU using pycocoevalcap (like eval_bleu.py).
    """
    ...
    tokenizer.padding_side = "left"  # consistent with pred_lisat_vqa
    img_processor = ImageProcessor(vision_tower.image_processor, image_size)

    # For predictions & references
    predictions_dict = {}
    references_dict = {}
    ...
    for line in tqdm.tqdm(lines, desc=f"Val-VQA ({os.path.basename(vqa_file)})"):
        ...
        base_dir = os.path.dirname(vqa_file)  # adjust if needed
