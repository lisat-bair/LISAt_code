...
def parse_args(args):
    ...
    parser.add_argument("--cmd", default="inference", type=str, help="Value can be: 'inference' (default), 'metrics', 'bootstrap_metrics', or 'download'.")
    ...
    parser.add_argument("--pretrained_model_path", default="/home/wenhan/Projects/sesame/runs/lisat_sam_7000")
...
def run_bootstrap_eval(preds, args, n_round=20, fraction=0.8):
    ...
    for i in range(n_round):
        ...
        print(f"[Round {i+1}/{n_round}] cIoU={ciou:.4f}, gIoU={giou:.4f}, SEE={see_acc:.4f}")
    ...
    print(f"SEE:  mean={see_mean:.4f}, std={see_std:.4f}")
...
def main(args):
    ...
    if args.cmd == "inference":
        ...
        print(f"[Final Result - {args.val_dataset}] cIoU: {ciou:.4f} | GIoU: {giou:.4f} | See Accuracy: {see_acc:.4f}")
