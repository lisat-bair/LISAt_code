...
from scipy.spatial import ConvexHull  # Import for GIoU computation


def parse_args(args):
    parser = argparse.ArgumentParser(description="SESAME's evaluation helper")
    parser.add_argument("--cmd", default="inference", type=str, help="Value can be: 'inference' (default), 'metrics', or 'download'.")
    ...
    parser.add_argument("--val_dataset", default="geo_reason_seg", type=str)
...
def inference(args):
    ...
    reason_seg_dataset = ["geo_reason_seg"]
    refer_seg_dataset = [
    ...
    for input_dict in tqdm.tqdm(test_loader):
        ...
        for n in range(num_batch):
            ...
            )  # real_batch_size x 3 x 336 x 336
            images = input_dict["images"].repeat(
                real_batch_size, 1, 1, 1
            )  # real_batch_size x 3 x 1024 x 1024
...
def run_eval(preds, args):
    # Initialize meters for cIoU, GIoU, and See Accuracy
    ...
    for idx, pred_key in enumerate(preds):
        ...
        for i in range(batch_size):
            ...
            acc_iou_i[union_i == 0] += 1.0  # Handle no-object target
        ...
        if idx % 50 == 49:
            # Compute average IoU and GIoU up to this batch
            iou_class = intersection_meter.sum / (union_meter.sum + 1e-10)
            ciou = iou_class[1]
            giou = acc_iou_meter.avg[1]
            detection_acc = correct_query / total_count
            print(
                f"[(batch {idx + 1}) - {args.val_dataset}] cIoU: {ciou:.4f} | GIoU: {giou:.4f} | See Accuracy: {detection_acc:.4f}"
            )
    ...
    detection_acc = correct_query / total_count
    print(
        f"[Final Result - {args.val_dataset}] cIoU: {ciou:.4f} | GIoU: {giou:.4f} | See Accuracy: {detection_acc:.4f}"
    )
