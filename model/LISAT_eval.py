    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not hasattr(self.config, "train_mask_decoder"):
            ...
            #self.config.out_dim = kwargs.get("out_dim", 768)
...
class LISATForCausalLM(LlavaLlamaForCausalLM):
    ...
    def generate_pred_masks(self, pred_embeddings, image_embeddings, sam_mask_shape_list):
        ...
        for i in range(len(pred_embeddings)):
            ...
            # Get input and original sizes
            input_size, original_size = sam_mask_shape_list[i][0], sam_mask_shape_list[i][1]

            # Convert sizes to integers if they are tensors
            ...
            if original_size[0] <= 0 or original_size[1] <= 0:
                ...
                pred_masks.append(torch.empty(0, *input_size, dtype=pred_embeddings[i].dtype, device=pred_embeddings[i].device))
                continue
...
def load_pretrained_model_LISAT(
    model_path,
    device_map="auto",
    device="cuda",
    **kwargs,
