...
class SESAMEForCausalLM(LlavaLlamaForCausalLM):
    ...
    def generate_pred_masks(self, pred_embeddings, image_embeddings, sam_mask_shape_list):
        ...
        for i in range(len(pred_embeddings)):
            ...
            if original_size[0] <= 0 or original_size[1] <= 0:
                pred_masks.append(
                    torch.empty(0, *input_size, dtype=pred_embeddings[i].dtype, device=pred_embeddings[i].device)
                )
                continue
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        # Pad the seg_token_mask so it accounts for all image patch tokens
        ...
        seg_token_mask = torch.cat([padding_left, seg_token_mask, padding_right], dim=1)
        ...
        # Convert [SEG]-token hidden states -> embeddings
        pred_embeddings = self.model.text_hidden_fcs[0](output_hidden_states)[seg_token_mask]
        ...
        # ---- Here's where we call self.get_visual_embs
    ...
    def get_visual_embs(self, pixel_values: torch.FloatTensor):
        """
        Moved inside SESAMEForCausalLM so that self.get_visual_embs is valid.
        """
        with torch.no_grad():
    ...
    def evaluate(
        ...
        with torch.inference_mode():
            ...
            # Check for any [SEG] tokens
            seg_token_mask = (output_ids[:, 1:] == self.seg_token_idx)
            if seg_token_mask.sum() == 0:
                # No [SEG] => pure text
                dummy_mask = [torch.zeros(1,1, dtype=torch.int).to(input_ids.device)]
                object_presence = [False]
                return output_ids, dummy_mask, object_presence

            # If we do have [SEG], proceed with segmentation logic
            vision_tower = self.get_vision_tower()
            num_tokens_per_image = vision_tower.num_patches
            padding_left = torch.zeros(
                seg_token_mask.shape[0],
...
def load_pretrained_model_SESAME(model_path, device_map="auto", device="cuda", **kwargs):
